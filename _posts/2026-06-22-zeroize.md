...
— doesn't actually keep the secret off the stack. `load_key()` returns the array *by value*, so the bytes first materialize in a stack temporary; and stable Rust has no placement-`new`, so `Box::new(x)` builds `x` on the stack and *then* copies it into the heap. You've just reintroduced exactly the by-value hop the demo punished. To squeeze out that last copy, allocate the heap slot *first* — zeroed, which is harmless on the stack — and fill it in place:
