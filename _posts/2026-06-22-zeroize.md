...
While it sleeps, a [simple script](https://github.com/f-squirrel/zeroize-demo) can scan `/proc/<pid>/mem` for the `DEADBEEF` pattern:
...
`drop(b)` zeroed exactly one location. Four copies of the secret are still sitting on the stack.
