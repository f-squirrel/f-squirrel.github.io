...
You can watch the difference for yourself: the same example has a [`heap_fix` binary](https://github.com/f-squirrel/f-squirrel.github.io/blob/master/examples/zeroize-demo/src/bin/heap_fix.rs) that does exactly this. Run `cargo run --bin heap_fix` and scan it the same way — you'll find a *single* copy on the heap while the key is live, and *zero* once it drops, versus the four stack fossils the move demo above leaves behind.
