...
You can watch the difference for yourself: the [`zeroize-demo` example](https://github.com/f-squirrel/f-squirrel.github.io/blob/master/examples/zeroize-demo/src/main.rs) runs either path on demand — `cargo run -- stack` for the leak above, `cargo run -- heap` for this fix. Scan the heap run the same way and you'll find a *single* copy while the key is live, and *zero* once it drops, versus the four stack fossils the move demo leaves behind.
