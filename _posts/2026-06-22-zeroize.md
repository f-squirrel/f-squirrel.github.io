...
- the value gets leaked — explicitly via `mem::forget` or `Box::leak`, or implicitly because the last reference is never dropped (e.g. an `Rc`/`Arc` cycle),
...
Run it with `cargo run -- stack` (it's the [`zeroize-demo` example](https://github.com/f-squirrel/f-squirrel.github.io/tree/master/examples/zeroize-demo)). While it sleeps, a [simple script](https://github.com/f-squirrel/f-squirrel.github.io/blob/master/examples/zeroize-demo/scan_mem.py) can scan `/proc/<pid>/mem` for the `DEADBEEF` pattern:
...
Now the secret lives at one fixed heap address. Passing `key` to a function, returning it, or tucking it into a struct moves the `Box` — eight bytes of pointer — while the 32 secret bytes stay exactly where they were allocated. There's only ever *one* copy at the storage level, and `Zeroizing` wipes it on drop. The whole class of "fossil on the stack" copies from the demo simply can't form, because nothing copies the bytes around anymore. (Register spills *during use* are a separate matter — the CPU can still drag the bytes through unnamed stack slots while you operate on them. We come back to that in "What no crate can quite reach.")
...
You can watch the difference for yourself: the [`zeroize-demo` example](https://github.com/f-squirrel/f-squirrel.github.io/blob/master/examples/zeroize-demo/src/main.rs) runs either path on demand — `cargo run -- stack` for the leak above, `cargo run -- heap` for this fix. The heap run parks itself in two halves so you can scan at both moments.

Start it, and it announces the PID and waits with the key live:

```text
$ cargo run -- heap
PID 9585 — secret is LIVE on the heap.
  Scan now — exactly ONE copy, no stack fossils:
```

In another terminal, scan while it's live:

```text
$ sudo python3 scan_mem.py 9585
  hit at 0x56081e800ae0 in [heap]
Found 1 occurrence(s) of the 32-byte DEADBEEF pattern in PID 9585
```

Then the demo drops the key and sleeps again. Scan once more:

```text
$ sudo python3 scan_mem.py 9585
Found 0 occurrence(s) of the 32-byte DEADBEEF pattern in PID 9585
```

One copy on the *heap* (not four scattered across the stack), and nothing left once it drops — exactly what we were after. (The address and PID are from one run; yours will differ.)
...
→ Next: [I Hardened My Secret. Or Did I?](/zeroize-os-hardening)
